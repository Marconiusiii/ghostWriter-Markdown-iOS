    ...
    static let all: [HelpTopic] = [
        ...
        HelpTopic(
            ...
            paragraphs: [
                ...
                "PowerPoint themes are Warm paper, Midnight, High contrast light, and High contrast dark. The selected theme applies to the whole presentation, and the app remembers your choice. Themes set the slide background, text, and link colors. They do not change the colors inside images.",
                "PowerPoint keeps bulleted and numbered lists, including nested levels. Task lists become list items beginning with Completed or Not completed, not interactive checkboxes. Links outside tables remain clickable. Quotes become paragraphs introduced by Quote, and code blocks become monospaced text without syntax highlighting.",
                "PowerPoint tables become labeled text rows, not native PowerPoint tables. Column headings label each row’s values. Table grids, column alignment, cell formatting, and cell-by-cell table navigation are not retained. Links inside table cells become plain text.",
                ...
                "PowerPoint allows up to four included images per slide. If a slide contains too much text or too many images, add another level 2 heading to divide the content. The exporter does not automatically split crowded slides.",
