    ...
    static let all: [HelpTopic] = [
        ...
        HelpTopic(
            ...
            paragraphs: [
                ...
                "EPUB creates an ebook that reflows to the text size chosen in the reading application. Headings at every level, lists, links, tables, quotes, code, and tasks retain their document structure. The file includes a hierarchical table of contents built from the document’s headings. Images imported with a Word document are stored inside the file. If an image cannot be packaged safely, its alternative text is included as readable text instead.",
                "eBraille creates a reflowable Unified English Braille document with a .ebrl filename. Headings, lists, links, tables, quotes, and ordinary images retain document structure. A hierarchical table of contents is built from the headings, and the reading application controls the displayed line length.",
                "The eBraille details screen asks for the braille grade, the author, who is producing this braille edition, a copyright date, and whether this is the complete document. These publication facts are required by eBraille and cannot all be inferred safely. A copyright date can be a year, year and month, or full date, such as 2026, 2026-04, or 2026-04-17. An impossible date is rejected rather than silently changed.",
                "Additional publication details can record a source work, the publisher of this eBraille edition, rights, subject, description, and education level. eBraille recommends these rather than requiring them, and empty fields are left out of the file.",
                "Insert Actions > Tactile Graphic attaches an SVG, PNG, or JPG prepared for tactile presentation and inserts its description using the optional Markdown title tactile. During eBraille export, the file is packaged at that location and the publication records its actual tactile graphic format. A tactile graphic must have a description and must be present and readable. Invalid image files and unsafe SVG files containing active or external content are rejected. Ordinary images are not described as tactile graphics, and an image with empty alternative text is decorative.",
                "Braille Ready Format creates fixed-layout Unified English Braille as a .brf file. Standard layout uses 40 cells by 25 lines; a custom display layout is also available. Lists use braille bullet and nested-runover conventions, tables become labeled rows, task items state Completed or Not completed, and tactile or ordinary images become labeled descriptions because BRF cannot contain image resources.",
                "Braille Ready Format keeps the readable link text but cannot preserve an active link destination unless the address is written visibly in the markdown. eBraille and EPUB keep active links.",
                "Grade 1 is uncontracted Unified English Braille and grade 2 is contracted Unified English Braille. Code blocks retain their line breaks and spacing and use uncontracted braille, but ghostWriter does not claim specialized computer-code transcription. These exports are intended for document sharing and personal reading, not as replacements for professional braille transcription software.",
                "The app remembers the braille grade, the producer name, and the last Braille Ready Format layout. Details particular to one document are cleared after the export screen closes so they are not reused accidentally.",
                "When an ordinary EPUB or eBraille image cannot be packaged from the document’s managed assets, its alternative text remains as readable fallback text. A declared tactile graphic is stricter: export stops if its file is missing, invalid, or unsafe rather than claiming that a tactile resource was included.",
