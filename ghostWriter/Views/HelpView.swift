    ...
    static let all: [HelpTopic] = [
        ...
        HelpTopic(
            ...
            paragraphs: [
                ...
                "eBraille creates a Unicode braille edition with a .ebrl filename. Headings at every level, lists, links, tables, and quotes retain their document structure. The file includes a hierarchical table of contents built from the document\u{2019}s headings, and text reflows to the line length of the display it is read on. Images imported with a Word document can be stored inside the file, with alternative text translated into braille. Ordinary pictures are not described as tactile graphics.",
                "The eBraille screen asks for the braille grade, an author, a transcriber, a copyright date, and whether the document is a complete transcription. The author is whoever wrote the original work; enter Unknown when the author is not known. The transcriber is the person or organization responsible for the braille transcription. These fields are required because ghostWriter cannot determine them accurately. The copyright date can be a year, a year and month, or a full date, such as 2026, 2026-04, or 2026-04-17.",
                "The Recommended details section records the source work being transcribed, the publisher, the rights, a subject, a description, and the education level the material was produced for. The eBraille standard recommends these rather than requiring them, and any you leave empty are left out of the file entirely.",
                "Braille Ready Format creates a fixed-page ASCII braille file with a .brf filename. The screen asks for the braille grade and the page size in cells per line and lines per page. Tables are written as a labelled line for each cell rather than as columns, and task items state Completed or Not completed.",
                "Grade 1 is uncontracted Unified English Braille and grade 2 is contracted Unified English Braille. eBraille reflows in its reading application; Braille Ready Format is wrapped to the selected page dimensions. The app remembers the braille grade, transcriber, and Braille Ready Format page dimensions. Details about a particular work are cleared after the export sheet closes so they cannot be reused in another document by mistake.",
                "Alternative text is carried into formats that support images. When an EPUB or eBraille image cannot be packaged from the document’s own imported assets, the export retains its alternative text instead of creating a broken or unsafe image reference. An image written with empty alternative text is omitted from that fallback.",
