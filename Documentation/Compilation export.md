...
Word and PDF offer Start each document on a new page. Turn it off for continuous output. Word uses an actual empty paragraph between documents, reusing existing blank paragraphs and preserving additional source blanks. PDF separates documents within its continuous page layout.
...
Markdown produces normalized Markdown with resolved reference links and blank-line separation. If it includes local images or linked files, the result is a ZIP containing the Markdown document and its assets. Extract it before opening the Markdown file, and keep the extracted files together. Plain Text removes Markdown syntax and preserves readable titles, lists, and table content.

PowerPoint offers Presentation theme and Font family. Each document starts its own slide. Heading 2 starts additional slides within that document; the standard PowerPoint rules for notes, images, tables, and slide capacity still apply. A document that exceeds a slide’s capacity must be divided with Heading 2 before exporting.

eBraille offers Braille code, Author, Produced by, Copyright date, and Complete transcription. Additional publication metadata is available in a disclosure. Required metadata must be valid before exporting. The selected braille code applies to the entire compilation, including documents with different source languages.

Braille Ready Format offers Braille code, Use, Layout, and custom cells-per-line and lines-per-page choices. Embossed Output also offers braille page numbers. Pagination and page numbering run across the combined output. The selected braille code applies to the whole compilation. Set physical paper and margins in your embossing software.
